    ...
    <div class="container mx-auto">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('dashboard.jobs.create') }}"
                class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-opacity-80 transition">
                ...
                <tbody>
                    @forelse ($jobs as $job)
                        <tr class="border-b border-dark-3">
                            ...
                            <td class="p-4">{{ Str::limit($job->job_description, 50) }}</td> <td class="p-4 text-center">
                            <td class="p-4 text-center">
                                <a href="{{ route('dashboard.jobs.edit', $job) }}"
                                    class="text-secondary hover:text-white mr-4"><i class="fas fa-edit"></i></a>
                                <form action="{{ route('dashboard.jobs.destroy', $job) }}" method="POST"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-accent hover:text-white"><i
